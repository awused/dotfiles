...
void main()
{
    ...
    if ( uvCoord.x < mpos.x+boxOffset.x+boxSize.x &&
      ...
      if ( crosshair && (
            (distance(uvCoord.x, mpos.x+boxOffset.x+boxSize.x/2) <= borderSize.x &&
              distance(uvCoord.y, mpos.y+boxOffset.y+boxSize.y/2) > 0.5/screenSize.y*magstrength) ||
            (distance(uvCoord.y, mpos.y+boxOffset.y+boxSize.y/2) <= borderSize.y &&
              distance(uvCoord.x, mpos.x+boxOffset.x+boxSize.x/2) > 0.5/screenSize.x*magstrength))) {
        color = borderColor;
      } else {
        // Calculate where the UV should be.
        vec2 zoomedUV = ((uvCoord-mpos)-(boxOffset+boxSize/2))/magstrength + vec2(0.5,-0.5)/screenSize +mpos;
        ...
        color = mix( texture2D( desktop, zoomedUVFlipped ), rectColor, 0.01 );
      }
