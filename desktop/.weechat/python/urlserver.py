...
urlserver = {
    ...
    re.compile(
        r'(\w+://(?:%s|%s)(?::\d+)?(?:/[^\])>\s]*)?)' %
        (url_domain, url_ipaddr), re.IGNORECASE),
    'urls': {},
    'number':
    0,
    'buffer':
...
urlserver_settings_default = {
    ...
    'http_hostname': ('', 'force hostname/IP in bind of socket '
    ...
    'http_allowed_ips': ('', 'regex for IPs allowed to use server '
    ...
    'http_embed_youtube':
    ('off', 'embed youtube videos in HTML page (BE CAREFUL: the HTTP referer '
     'will be sent to youtube!)'),
    'http_embed_youtube_size':
    ('480*350', 'size for embedded youtube video, format is "xxx*yyy"'),
    'http_prefix_suffix':
    ...
    'display_urls_in_msg':
    ('off', 'add shorten url next to the original url (only in IRC messages) '
     '(useful for urlserver behind relay/irc)'),
    'url_min_length':
    ('0', 'minimum length for an URL to be shortened (0 = shorten all URLs, '
     '-1 = detect length based on shorten URL)'),
    'urls_amount':
    ('100', 'number of URLs to keep in memory (and in file when script is not '
     'loaded)'),
...
def urlserver_get_base_url():
    ...
    hostname = (urlserver_settings['http_hostname_display']
                or urlserver_settings['http_hostname'] or socket.getfqdn())
...
def urlserver_server_reply(conn,
                           code,
                           extra_header,
                           message,
                           mimetype='text/html'):
...
def urlserver_server_reply_list(conn, sort='-time'):
    ...
    if sort[1:] == 'time':
        ...
        urls = sorted(
            urlserver['urls'].items(), key=lambda url: url[1][idx].lower())
    if sort.startswith('-'):
        urls.reverse()
    sortkey = {'-': ('', '&uarr;'), '+': ('-', '&darr;')}
    content += '  <tr>'
    ...
    for key, item in urls:
        ...
        url = item[3]
        obj = ''
        message = (cgi.escape(item[4].replace(url, '\x01\x02\x03\x04')).split(
            '\t', 1))
        ...
        message = strjoin.join(message).replace(
            ...
            '</a><span class="message">' % (urlserver_short_url(key, False),
                                            url, target, url))
    ...
    html = ('<html>\n'
            ...
            '</head>\n'
            '<body>\n%s\n</body>\n'
            '</html>' % (urlserver_settings['http_title'], css,
                         urlserver_get_base_url(), content))
...
def urlserver_server_status():
    ...
    if urlserver['socket']:
        weechat.prnt(
            '', 'URL server listening on %s' % str(
                urlserver['socket'].getsockname()))
    else:
...
def urlserver_display_url_detail(key, return_url=False):
    ...
    if return_url:
        ...
        weechat.prnt_date_tags(
            urlserver['buffer'], 0, 'notify_none', '%s, %s%s%s%s: %s%s%s -> %s'
...
def urlserver_open_buffer():
    ...
    if not urlserver['buffer']:
        urlserver['buffer'] = weechat.buffer_new(
            SCRIPT_BUFFER, 'urlserver_buffer_input_cb', '',
...
def urlserver_update_urllist(buffer_full_name,
                             buffer_short_name,
                             tags,
                             prefix,
                             message,
    ...
    for url in urlserver['regex'].findall(message):
        if len(url) >= min_length:
            ...
            if not url.startswith(urlserver_get_base_url()):
                urlserver['urls'][number] = (datetime.datetime.now().strftime(
                    urlserver_settings['http_time_format']), nick,
                                             buffer_short_name, url,
                                             '%s\t%s' % (prefix, message))
...
def urlserver_print_cb(data, buffer, time, tags, displayed, highlight, prefix,
    ...
    if urlserver_settings['display_urls'] == 'on':
        ...
        urls_short = urlserver_update_urllist(
            buffer_full_name, buffer_short_name, tags, prefix, message)
        if urls_short:
            ...
            weechat.prnt_date_tags(
                buffer, 0, 'no_log,notify_none', '%s%s' % (weechat.color(
                    urlserver_settings['color']), urls_string))
...
def urlserver_modifier_irc_cb(data, modifier, modifier_data, string):
    ...
    if urlserver_settings['buffer_short_name'] == 'on':
        ...
        buffer_short_name = buffer_full_name
    urls_short = urlserver_update_urllist(buffer_full_name, buffer_short_name,
                                          None, msg['nick'], message,
                                          msg['nick'])
    if urls_short:
        if urlserver_settings['separators'] and \
                len(urlserver_settings['separators']) == 3:
            separator = ' %s ' % (urlserver_settings['separators'][1])
            urls_string = separator.join(urls_short)
            urls_string = '%s %s %s' % (urlserver_settings['separators'][0],
                                        urls_string,
                                        urlserver_settings['separators'][2])
        else:
            urls_string = ' | '.join(urls_short)
...
def urlserver_filename():
    ...
    return os.path.join(
        weechat.info_get('weechat_dir', ''), 'urlserver_list.txt')
